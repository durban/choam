...
import scala.scalanative.unsafe.{ extern, link, CSize, CVoidPtr, CInt, Ptr, UnsafeRichArray }
...
private final class NativeOsRng extends OsRng {
  ...
  final override def nextBytes(dest: Array[Byte]): Unit = {
    ...
    if (LinktimeInfo.isWindows) {
      nextBytesWin(ptr, dest.length.toUInt)
    } else {
      nextBytesUnix(ptr, dest.length.toCSize)
    }
  }

  @tailrec
  private[this] final def nextBytesUnix(start: Ptr[Byte], remaining: CSize): Unit = {
    val max = 256.toUSize
    val size = if (remaining > max) max else remaining
    getentropy(start, size)
    val rem = remaining - size
    if (rem > 0.toUSize) {
      nextBytesUnix(start + size, rem)
    } // else: we're done
  }

  private[this] final def getentropy(buffer: CVoidPtr, length: CSize): Unit = {
    val res: CInt = unistd.getentropy(buffer, length)
    if (res != 0) {
  ...
  private[this] final def nextBytesWin(buffer: CVoidPtr, length: UInt): Unit = {
    val status: UInt = Bcrypt.BCryptGenRandom(
      null,
      buffer,
      length,
      0x00000002.toUInt, // BCRYPT_USE_SYSTEM_PREFERRED_RNG
    )
