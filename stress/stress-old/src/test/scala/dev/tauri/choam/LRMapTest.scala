...
class LRMapTest extends StressTestBase {
  ...
  private[this] def rxn1(r: LLLLLL_Result): Axn[String] = {
    ref1.getAndUpdate(_ + "b").map { s =>
      r.r1 = Rxn.unsafe.directRead(ref1).unsafeRun(this.impl) // this is cheating
      r.r2 = Rxn.unsafe.directRead(ref2).unsafeRun(this.impl) // this is cheating
      s
    }
  }
  ...
  private[this] def rxn2(r: LLLLLL_Result): Rxn[String, String] = Rxn.computed { (s: String) =>
    ...
    r.r3 = Rxn.unsafe.directRead(ref1).unsafeRun(this.impl) // this is cheating
    r.r4 = Rxn.unsafe.directRead(ref2).unsafeRun(this.impl) // this is cheating
    s
  }
