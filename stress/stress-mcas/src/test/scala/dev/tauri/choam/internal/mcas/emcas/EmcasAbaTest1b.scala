...
@Outcomes(Array(
  ...
  new Outcome(id = Array("a, y, true, true, y, x"), expect = ACCEPTABLE_INTERESTING, desc = "FORBIDDEN: non-linearizable (and t1 reads its own result)"),
  // Note: a non-linearizable result with t1 reading t2's result
