...
object RxnModelTest {
  ...
  class TestState {
    ...
    private[this] val r1 =
      Ref.unsafe("a", AllocationStrategy.Unpadded, emcas.currentContext().refIdGen)

    private[this] val r2 =
      Ref.unsafe("b", AllocationStrategy.Unpadded, emcas.currentContext().refIdGen)

    private[this] val r3 =
      Ref.unsafe("c", AllocationStrategy.Unpadded, emcas.currentContext().refIdGen)
