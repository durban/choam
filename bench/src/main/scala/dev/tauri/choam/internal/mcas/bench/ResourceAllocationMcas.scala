  ...
  def bench(s: RaSt, t: ThSt, rnd: RandomState): Unit = {
    ...
    def read(i: Int): Unit = {
      if (i >= n) {
        ...
        val Some((_, newDesc: AbstractDescriptor)) = ctx.readMaybeFromLog(rss(i), t.desc, canExtend = true) : @unchecked
        t.desc = newDesc
        read(i + 1)
      }
    }
...
object ResourceAllocationMcas {
  ...
  class RaSt extends McasImplStateBase {
    ...
    val rss: Array[MemoryLocation[String]] =
      initialValues.map(Ref.unsafe(_, AllocationStrategy.Padded, this.mcasImpl.currentContext().refIdGen).loc).toArray
