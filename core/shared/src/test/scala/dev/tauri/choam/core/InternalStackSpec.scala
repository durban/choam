...
final class InternalStackSpec extends BaseSpecA {
  ...
  test("ObjStack") {
    ...
    s.push("a", ObjStack.Freelist.Dummy)
    assertEquals(s.toString, "ObjStack(a)")
    assert(!s.isEmpty)
    assert(s.nonEmpty)
    val s1 = s.takeSnapshot()
    s.push("b", ObjStack.Freelist.Dummy)
    assertEquals(s.toString, "ObjStack(b, a)")
    s.pushAll("c" :: "d" :: Nil, ObjStack.Freelist.Dummy)
    assertEquals(s.toString, "ObjStack(d, c, b, a)")
    val s2 = s.takeSnapshot()
    s.loadSnapshot(s1)
    assertSameInstance(s.pop(ObjStack.Freelist.Dummy), "a")
    assert(s.isEmpty)
    s.loadSnapshot(s2)
    assertSameInstance(s.pop(ObjStack.Freelist.Dummy), "d")
    assertSameInstance(s.pop(ObjStack.Freelist.Dummy), "c")
    s.clear()
    assert(s.isEmpty)
    s.loadSnapshot(s0)
    assert(s.isEmpty)
    assert(Try { s.pop(ObjStack.Freelist.Dummy) }.isFailure)
  }
