...
final class InteropSpec extends CatsEffectSuite with MUnitUtils {
  ...
  private[this] val _resAndClose: (Reactive[IO], SyncIO[Unit]) =
    Reactive.fromIn[SyncIO, IO](unsafeRuntime).allocated.unsafeRunSync()

  private implicit def F: Reactive[IO] =
    _resAndClose._1

  final override def afterAll(): Unit = {
    _resAndClose._2.unsafeRunSync()
    super.afterAll()
  }
