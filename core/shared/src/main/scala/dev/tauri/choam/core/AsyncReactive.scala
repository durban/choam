...
sealed trait AsyncReactive[F[_]] extends Reactive.UnsealedReactive[F] { self =>
  def applyAsync[A, B](r: Rxn[B], a: A /* TODO: <- remove this */, s: RetryStrategy = RetryStrategy.Default): F[B]
...
object AsyncReactive {
  ...
  private[choam] class AsyncReactiveImpl[F[_]](mi: Mcas)(implicit F: Async[F])
    ...
    final override def applyAsync[A, B](r: Rxn[B], a: A, s: RetryStrategy = RetryStrategy.Default): F[B] =
