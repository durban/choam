...
import MichaelScottQueueUnpadded._

private[choam] final class MichaelScottQueueUnpadded[A] private[this] (sentinel: Node[A], els: Iterable[A])
  extends Queue[A] {
  ...
  override val enqueue: Rxn[A, Unit] = Rxn.computed { (a: A) =>
    findAndEnqueue(Node(a, Ref.unsafeUnpadded(End[A]())))
  }
...
private[choam] object MichaelScottQueueUnpadded {
  ...
  def apply[A]: Axn[MichaelScottQueueUnpadded[A]] =
    Rxn.unsafe.delay { _ => new MichaelScottQueueUnpadded }

  def fromList[A](as: List[A]): Axn[MichaelScottQueueUnpadded[A]] =
    Rxn.unsafe.delay { _ => new MichaelScottQueueUnpadded(as) }
}
